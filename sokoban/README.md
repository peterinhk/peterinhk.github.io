...
A modern implementation of the classic Sokoban puzzle game using HTML5 Canvas and JavaScript, from [klevze/sokoban](https://github.com/klevze/sokoban).

This directory contains the production build (`npm run build`, via Vite) with a
small patch on top: the original "tap anywhere" advance past the
loading/intro/win screens only worked when the tap landed on the game canvas
itself, but the mobile virtual-controls container sits on top of the canvas
and intercepted the touch first, silently trapping mobile players on the
"Press Space to Load Game" title screen. The fix moves that advance-on-tap
listener to `document` so it fires regardless of which element the touch
hit. See the upstream repo for the full source and asset files.
